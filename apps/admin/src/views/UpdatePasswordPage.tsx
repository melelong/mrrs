  ...
  setup() {
    ...
    const getUpdateUserPasswordCaptcha = async () => {
      try {
        ...
        const { data } = await adminApi.user.userUpdateUserPasswordCaptcha(
          updateUserPasswordFormState.email
        )
    ...
    return () => (
      <div class="updateUserPassword_container w-screen h-screen flex items-center justify-center flex-col text-center select-none bg-bg bg-repeat">
        <div class="form_container 2xl:w-3/12 2xl:h-2/12 xl:w-4/12 xl:h-6/12 lg:w-5/12 lg:h-6/12 md:w-6/12 md:h-5/12 sm:w-7/12 sm:h-5/12 p-8 rounded-xl border shadow-2xl bg-white">
          ...
          >
            ...
            <Form.Item name="username">
              <Input
                placeholder="用户名"
                v-model={[updateUserPasswordFormState.username, 'value']}
            ...
            <Form.Item name="password">
              <Input.Password
                placeholder="新密码"
                v-model={[updateUserPasswordFormState.password, 'value']}
            ...
            <Form.Item name="email">
              <Input
                placeholder="邮箱地址"
                v-model={[updateUserPasswordFormState.email, 'value']}
            ...
            <div class="w-full flex">
              <Form.Item name="captcha">
                <Input
                  placeholder="验证码"
                  v-model={[updateUserPasswordFormState.captcha, 'value']}
              ...
              <Button
                class="ml-4"
                disabled={isGetUpdateUserPasswordCaptcha.value}
                loading={isGetUpdateUserPasswordCaptchaLoading.value}
                onClick={getUpdateUserPasswordCaptcha}
                type="primary"
              >
                发送验证码
              </Button>
