...
const isGetUpdateUserPasswordCaptcha = computed<boolean>(
  () =>
    !(
      ...
      updateUserPasswordFormState.captcha.length !== 6 &&
      confirmPwd.value
    )
)
// 发送验证码按钮loading状态
...
const handleFinish = async () => {
  try {
    ...
    const { data } = await userApi.user.userUpdateUserPassword(updateUserPasswordFormState)
...
const getUpdateUserPasswordCaptcha = async () => {
  try {
    ...
    const { data } = await userApi.email.updatePasswordCaptcha(updateUserPasswordFormState.email)
