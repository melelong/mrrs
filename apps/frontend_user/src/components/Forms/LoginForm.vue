...
import { userApi, svgToBase64 } from '@/uitls'
import {
  useFormState,
  useFormPass,
  ...
  try {
    ...
    const {
      data: {
        refreshToken,
        accessToken,
        userInfo: { roles, permissions, ...userInfo }
      }
    } = await userApi.user.userLogin(loginFormState)
    isLoading.value = false
    message.success('登录成功')
    tokenState.setUserInfo(userInfo)
    tokenState.setRoles(roles)
    tokenState.setPermissions(permissions)
...
const getUserLoginCaptcha = async () => {
  try {
    const { data } = await userApi.email.userLoginCaptcha()
...
<template>
  ...
  >
    <!-- 标题 -->
    <h1 class="text-[#1976D2] font-bold text-3xl mb-8 mt-4">会议室预订系统(用户端)</h1>
    ...
    <div class="w-full flex">
      <Space direction="horizontal">
        ...
        <span @click="getUserLoginCaptcha">
          <Image
